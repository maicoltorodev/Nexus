...
export default function FloatingButton() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Aparece cuando se baja más de la altura del hero
      setIsVisible(scrollPosition > heroHeight * 0.8);

      // Solo mostrar mensaje de servicios si NO se está mostrando el de calculadora
      if (!showCalculatorText) {
        // Detectar si está en la sección de servicios
        ...
        if (serviciosSection) {
          ...
          // Mostrar texto si la sección de servicios está visible en el viewport
          const isInViewport = viewportTop < serviciosBottom && viewportBottom > serviciosTop;
          setShowHelpText(isInViewport && scrollPosition > heroHeight * 0.8);
        } else {
          setShowHelpText(false);
        }
      } else {
        // Si se está mostrando el de calculadora, ocultar el de servicios
        setShowHelpText(false);
      }
    };
  ...
  return (
    <div className="fixed bottom-8 right-8 z-50 flex items-center gap-3">
      ...
      <button
        ...
        className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-[#FFD700] to-[#FFA500] shadow-2xl border-2 border-[#FFD700]/50 transition-all duration-500 ${
